...
double opt3001_get_data(I2C_Handle *i2c) {
    ...
    // JTKJ: Exercise 2. Modify the function to return the measured value as lux

    double lux = -1.0; // Return value of the function
    uint8_t txBuffer[2]; // Tx buffer to send the OPT3001 register address
    uint8_t rxBuffer[2]; // Rx buffer to receive the sensor data

    I2C_Transaction i2cMessage;
    
    // Prepare the transaction to read data from OPT3001
    i2cMessage.slaveAddress = Board_OPT3001_ADDR;
    txBuffer[0] = OPT3001_REG_RESULT; // Register address for sensor data
    ...
    if (opt3001_get_status(i2c) & OPT3001_DATA_READY) {
        if (I2C_transfer(*i2c, &i2cMessage)) {
            // JTKJ: Here, perform the conversion from register value to lux
            // The conversion algorithm should be added here, using the values in rxBuffer

            // Extract the data from the rxBuffer and convert it to lux
            uint16_t rawLux = (rxBuffer[0] << 8) | rxBuffer[1];
            // The conversion formula may look like this, you should check the sensor's datasheet for the correct calculation:
            // lux = <your conversion formula>(rawLux);
            lux = (double)rawLux / 100.0;
            // Now, the 'lux' variable should contain the sensor data in lux units
        } else {
